...
setInterval(() => {
  http.get(`http://redbot234.glitch.me/`);
}, 280000);
...
const dev = ["708243857917345813"]; ///تعديل مهم حط ايدي حسابك
const admin = "=";
var owner = "708243857917345813"; // Owner Bot Mention /// تعديل مهم حط ايدي حسابكتعديل مهم ايدي حسابك
...
client.on("ready", () => {
  ...
  console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
  client.user.setActivity("Red Bot" ); ///تعديل مهم غير كلمة هيلب وبرميوم اللي بدك اياه مثل اسم سيرفرك
...
client.on("message", zaid => {
  if (zaid.content === "=bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@708243857917345813>`, true) // تعديل مهم عدل هذا الرقم لايدي حسابك
      .setImage(
        ""
      )
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "كيف اضيف البوت")) {
    var mbot = message.mentions.members.first();
    message.channel.send(`
https://discord.com/api/oauth2/authorize?client_id=733710488126488677&permissions=8&scope=bot تفضل هاذا رابط اضافه البوت`
    );
  }
});
...
client.on("message", message => {
  if (message.content.startsWith(`<@${client.user.id}>`)) {
    ...
    let mention = new Discord.RichEmbed()
      ...
      .setDescription(
        ` ✽  **Hi I'm Red Bot**  
✽  **Support Server** [ https://discord.gg/fP29YT6 ] 
✽  **Bot orders** [ • **=help** • ]   `
      )
...
client.on("message", message => {
  ...
  if (message.content === prefix + "1help") {
    message.channel.send(
      `**:smiling_imp:  | تم ارسال الاوامر العاديه  على الخاص **`
    );

    message.author.sendMessage(` ✽ **__ Red  Bot v1__**
**__الاوامر العامه__** 
...
client.on("message", message => {
  ...
  if (message.content === prefix + "2help") {
    message.channel.send(
      `**:smiling_imp:  | تم ارسال الاوامر الإداره على الخاص **`
    );
    
    message.author.sendMessage(` **✽ Red Bot v1**
...
client.on("message", message => {
  ...
  if (message.content === prefix + "3help") {
    message.channel.send(
      `** :smiling_imp: | تم ارسال اوامر الاغاني  في الخاص**`
    );

    message.author.sendMessage(`  **Red Bot v1**
  **__أوامر الاغاني__**   
**  حاليا البوت لا يدعم الموسيقى ،  **ا
** -   ا **
...
client.on("message", message => {
  ...
  if (message.content === prefix + "5help") {
    ...
    message.channel.send(
      `**:smiling_imp:  | تم ارسال الاوامر الحلوه  على الخاص **`
    );
...
> ** =1help ** ** الاوامر العاديه  **
> ** =2help ** ** الاوامر الإداره  **
> ** =3help ** ** أوامر الاغاني   **
